...
describe('auditSearchRequest', () => {
  ...
  it('logs an audit event with details from the request and user', async () => {
    await auditSearchRequest({ services, page: Page.VIEW_DPS_USER })(req, res, next)

    expect(logAuditEvent).toHaveBeenCalledWith({
      correlationId: 'request123',
      who: 'user1',
      what: Page.VIEW_DPS_USER,
  ...
  it('does not audit when search query body param is missing', async () => {
    ...
    await auditSearchRequest({ services, page: Page.VIEW_DPS_USER })(reqWithoutSearchTerm, res, next)
