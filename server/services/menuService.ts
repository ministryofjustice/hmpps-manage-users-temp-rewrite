...
export default class MenuService {
  ...
  getTiles(userRoles: string[]): MenuTile[] {
    return userRoles
      .flatMap(userRole => {
        return this.getAllTiles().filter(tile => tile.rolePredicate(userRole))
      })
  ...
  allowlistEnabled: boolean = manageUserAllowListHelper.isEnabled()

  allowlistEnvironment = manageUserAllowListHelper.environmentLabel()

  manageAllowlistRoles = ['MANAGE_USER_ALLOW_LIST']

  manageAllowlistRolePredicate: (role: string) => boolean = role =>
    this.manageAllowlistRoles.includes(role) && this.allowlistEnabled

  private getAllTiles(): RoleSpecificMenuTile[] {
    return [
