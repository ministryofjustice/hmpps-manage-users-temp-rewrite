...
test.describe('Health', () => {
  ...
  test.describe('All healthy', () => {
    test.beforeEach(async () => {
      await Promise.all([hmppsAuth.stubPing(), manageUsersApi.stubPing(), tokenVerification.stubPing()])
    })
    ...
    test('Info is accessible', async ({ page }) => {
      ...
      const payload = await response.json()
      expect(payload.build.name).toBe('hmpps-manage-users-temp-rewrite')
    })
  })

  test.describe('Some unhealthy', () => {
    test.beforeEach(async () => {
      await Promise.all([hmppsAuth.stubPing(), manageUsersApi.stubPing(503), tokenVerification.stubPing(500)])
    })
    ...
    test('Health check status is down', async ({ page }) => {
      ...
      expect(payload.components.manageUsersApi.status).toBe('DOWN')
      expect(payload.components.manageUsersApi.details.status).toBe(503)
      expect(payload.components.manageUsersApi.details.attempts).toBe(3)
